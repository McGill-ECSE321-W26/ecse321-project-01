        ...
        <div class="px-8 pt-8 pb-8">
          ...
          <div class="grid grid-cols-[2fr_2fr_1fr_1fr] gap-0 border border-(--text-light) mb-8">
            <div class="stat-card p-6 border-r border-(--text-light)" style="animation-delay: 0s">
              <p class="text-[10px] uppercase tracking-[0.2em] text-(--text-light) mb-3">Order ID</p>
              <p class="text-[11px] font-light text-(--text) font-mono leading-relaxed break-all">{{ selectedOrder.orderID }}</p>
            </div>
            <div class="stat-card p-6 border-r border-(--text-light)" style="animation-delay: 0.07s">
              ...
              <p class="text-[13px] font-light text-(--text) leading-snug">{{ selectedOrder.address }}</p>
            </div>
            <div class="stat-card p-6 border-r border-(--text-light)" style="animation-delay: 0.14s">
              <p class="text-[10px] uppercase tracking-[0.2em] text-(--text-light) mb-3">Total</p>
              <p class="text-[32px] font-light text-(--text) leading-none">${{ selectedOrder.totalPrice.toFixed(2) }}</p>
            </div>
            <div class="stat-card p-6" style="animation-delay: 0.21s">
              <p class="text-[10px] uppercase tracking-[0.2em] text-(--text-light) mb-3">Money Saved</p>
              <p class="text-[32px] font-light text-(--text) leading-none">${{ (selectedOrder.loyaltySaving ?? 0).toFixed(2) }}</p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-[1fr_1fr_auto] gap-0 border border-(--text-light) mb-8">
            <div class="p-6 border-r border-(--text-light)">
              <p class="text-[10px] uppercase tracking-[0.2em] text-(--text-light) mb-3">Status</p>
              <span
            ...
            <div class="p-6 border-r border-(--text-light)">
              <p class="text-[10px] uppercase tracking-[0.2em] text-(--text-light) mb-3">Employee</p>
              <span v-if="selectedOrder.employeeID" class="text-[13px] font-light text-(--text) font-mono">#{{ selectedOrder.employeeID }}</span>
              <span v-else class="text-[13px] font-light text-(--text-light)">Unassigned</span>
            </div>
          ...
          <!-- Items table -->
          <div class="stat-card border border-(--text-light)" style="animation-delay: 0.28s">
            ...
            <div v-if="detailLoading" class="px-6 py-10 text-[13px] text-(--text-light)">Loading items…</div>
            <div v-else-if="detailError" class="px-6 py-10 text-[13px] text-red-400">{{ detailError }}</div>
            <div v-else-if="detailItems.length === 0" class="px-6 py-10 text-[13px] text-(--text-light)">No items found.</div>
